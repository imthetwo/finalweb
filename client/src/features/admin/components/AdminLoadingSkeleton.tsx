    ...
    <div className="flex min-h-screen bg-base">
      ...
      <aside className="flex w-56 shrink-0 flex-col border-r border-edge bg-surface">
        <div className="border-b border-edge px-5 py-5">
          <div
            className="h-2 w-16 animate-shimmer rounded bg-brand/20 bg-[length:200%_100%]"
            style={{ backgroundImage: "var(--gradient-shimmer-brand)" }}
          />
        ...
        <div className="flex-1 space-y-1.5 p-3">
          {Array.from({ length: 7 }).map((_, i) => (
            <div
              key={i}
              className="h-9 animate-pulse rounded bg-elevated/60"
